...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.upcoming.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.upcomings.update", [$upcoming->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('car') ? 'is-invalid' : '' }}" name="car_id" id="car_id" required>
                        @foreach($cars as $id => $car)
                            <option value="{{ $id }}" {{ (old('car_id') ? old('car_id') : $upcoming->car->id ?? '') == $id ? 'selected' : '' }}>{{ $car }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('car'))
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $upcoming->description) !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('mileage') ? 'is-invalid' : '' }}" type="number" name="mileage" id="mileage" value="{{ old('mileage', $upcoming->mileage) }}" step="1">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control datetime {{ $errors->has('repair_at') ? 'is-invalid' : '' }}" type="text" name="repair_at" id="repair_at" value="{{ old('repair_at', $upcoming->repair_at) }}" required>
