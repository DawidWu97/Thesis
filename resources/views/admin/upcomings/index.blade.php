...
@section('content')
    @can('upcoming_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.upcomings.create') }}">
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.upcoming.title_singular') }} {{ trans('global.list') }}
        </div>

        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-Upcoming">
                    <thead>
                    <tr>
                        <th width="10">
                    ...
                    @foreach($upcomings as $key => $upcoming)
                        <tr data-entry-id="{{ $upcoming->id }}">
                            ...
                            <td>
                                @can('upcoming_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.upcomings.show', $upcoming->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('upcoming_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.upcomings.edit', $upcoming->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
...
@section('scripts')
    ...
    <script>
        $(function () {
            ...
            let table = $('.datatable-Upcoming:not(.ajaxTable)').DataTable({ buttons: dtButtons })
