@can('service_station_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.service-stations.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.serviceStation.title_singular') }}
            </a>
        </div>
    </div>
@endcan

<div class="card">
    <div class="card-header">
        {{ trans('cruds.serviceStation.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-userServiceStations">
                <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.id') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.user') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.name') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.phone') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.photo') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.opening') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.closing') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.workplaces') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.city') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.street') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.postcode') }}
                    </th>
                    <th>
                        {{ trans('cruds.serviceStation.fields.approved') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
                </tr>
                </thead>
                <tbody>
                @foreach($serviceStations as $key => $serviceStation)
                    <tr data-entry-id="{{ $serviceStation->id }}">
                        <td>

                        </td>
                        <td>
                            {{ $serviceStation->id ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->user->username ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->name ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->phone ?? '' }}
                        </td>
                        <td>
                            @if($serviceStation->photo)
                                <a href="{{ $serviceStation->photo->getUrl() }}" target="_blank" style="display: inline-block">
                                    <img src="{{ $serviceStation->photo->getUrl('thumb') }}">
                                </a>
                            @endif
                        </td>
                        <td>
                            {{ $serviceStation->opening ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->closing ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->workplaces ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->city ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->street ?? '' }}
                        </td>
                        <td>
                            {{ $serviceStation->postcode ?? '' }}
                        </td>
                        <td>
                            <span style="display:none">{{ $serviceStation->approved ?? '' }}</span>
                            <input type="checkbox" disabled="disabled" {{ $serviceStation->approved ? 'checked' : '' }}>
                        </td>
                        <td>
                            @can('service_station_show')
                                <a class="btn btn-xs btn-primary" href="{{ route('admin.service-stations.show', $serviceStation->id) }}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan

                            @can('service_station_edit')
                                <a class="btn btn-xs btn-info" href="{{ route('admin.service-stations.edit', $serviceStation->id) }}">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan

                            @can('service_station_delete')
                                <form action="{{ route('admin.service-stations.destroy', $serviceStation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                    <input type="hidden" name="_method" value="DELETE">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                </form>
                            @endcan

                        </td>

                    </tr>
                @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>

@section('scripts')
    @parent
    <script>
        $(function () {
            let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
            @can('service_station_delete')
            let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
            let deleteButton = {
                text: deleteButtonTrans,
                url: "{{ route('admin.service-stations.massDestroy') }}",
                className: 'btn-danger',
                action: function (e, dt, node, config) {
                    var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
                        return $(entry).data('entry-id')
                    });

                    if (ids.length === 0) {
                        alert('{{ trans('global.datatables.zero_selected') }}')

                        return
                    }

                    if (confirm('{{ trans('global.areYouSure') }}')) {
                        $.ajax({
                            headers: {'x-csrf-token': _token},
                            method: 'POST',
                            url: config.url,
                            data: { ids: ids, _method: 'DELETE' }})
                            .done(function () { location.reload() })
                    }
                }
            }
            dtButtons.push(deleteButton)
            @endcan

            $.extend(true, $.fn.dataTable.defaults, {
                orderCellsTop: true,
                order: [[ 1, 'asc' ]],
                pageLength: 100,
            });
            let table = $('.datatable-userServiceStations:not(.ajaxTable)').DataTable({ buttons: dtButtons })
            $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
                $($.fn.dataTable.tables(true)).DataTable()
                    .columns.adjust();
            });

        })

    </script>
@endsection
