                            ...
                            <table class=" table table-bordered table-striped table-hover datatable">
                                ...
                                @foreach($tasks as $key => $task)
                                    <tr data-entry-id="{{ $task->id }}">
                                        <td>
                                            {{ Illuminate\Support\Facades\DB::table('cars')->where('id', $task->repair->car_id)->value('brand') ?? '' }}
                                        </td>
                                        <td>
                                            {{ Illuminate\Support\Facades\DB::table('cars')->where('id', $task->repair->car_id)->value('model') ?? '' }}
                                        </td>
                                        <td>
                                            {{ Illuminate\Support\Facades\DB::table('cars')->where('id', $task->repair->car_id)->value('engine') ?? '' }}
                                        </td>
                                        <td>
                                            {{ $task->name ?? '' }}
                                        </td>
...
@section('scripts')
    ...
    <script>
        $(function () {
            ...
            let table = $('.datatable:not(.ajaxTable)').DataTable({ buttons: dtButtons })
            $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
                $($.fn.dataTable.tables(true)).DataTable()
                    .columns.adjust();
            });
