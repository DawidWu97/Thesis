...
@section('content')
    @can('user_request_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.user-requests.create') }}">
                        ...
                        <tr data-entry-id="{{ $userRequest->id }}">
                            ...
                            <td>
                                @can('user_request_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.user-requests.show', $userRequest->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('user_request_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.user-requests.edit', $userRequest->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('user_request_delete')
                                    <form action="{{ route('admin.user-requests.destroy', $userRequest->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
    ...
    <script>
        $(function () {
            ...
            let deleteButton = {
                ...
                url: "{{ route('admin.user-requests.massDestroy') }}",
